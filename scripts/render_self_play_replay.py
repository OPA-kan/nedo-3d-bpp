...
def build_payload(manifest_path: pathlib.Path, game_index: int) -> dict[str, Any]:
    ...
    audits = {
        ...
        for row in game.get("legal_move_audits", [])
    }
    ...
    for capture in sorted(game.get("captures", []), key=lambda row: row["step"]):
        ...
        frames.append({
            ...
            "proposal_count": int(
                record.get(
                    "proposal_count",
                    legal_audit.get("proposal_count", record.get("candidate_count", 0)),
                )
            ),
            "prefilter_rejections": int(
                legal_audit.get("rejected_count", 0)
            ),
