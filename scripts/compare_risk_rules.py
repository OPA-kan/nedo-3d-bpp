...
def render_markdown(result: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "closely, so P_hat is usable as a probability. Within the range "
        "compared here, no nonlinear rule beat the linear one. That "
        "removes one motivation for nonlinearity (miscalibration), but "
        "not the others: candidate-dependent loss L(s,a), different "
        "damage for large vs small rotations, nonlinear risk tolerance, "
        ...
        "enforce gate (different features, thresholds, and fallback "
        "design) nor online enforcement: live candidate generation and "
        "search cutoffs change the candidate set, so online starvation "
        "is not ruled out.",
        ...
        "cannot test state-dependent lambda(s) -- and remaining items "
        "is itself an unverified proxy for the failure loss "
        "V_safe - V_fail. Early snapshots would be added to identify "
        "whether the failure loss varies with state, not merely to "
        ...
        "than Q - lambda*P_hat on current data.",
        "",
    ]
    return "\n".join(lines) + "\n"
