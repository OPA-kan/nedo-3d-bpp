...
_production_transport_samples = _production.transport_samples
_production_transport_sweeps = _production.transport_sweeps
_production_action_center = _production.simulator_action_center


def simulator_action_center(candidate, container):
    """Rule-alpha release pose, compatible with both production revisions.

    The source branch already taught the production helper about the 2 cm
    floor release.  The Cup trunk predates that narrow helper change, so add
    it here without altering the shipped current agent.  Shelf releases remain
    wholly owned by the production helper.
    """
    action_center = _production_action_center(candidate, container)
    if abs(float(action_center[2]) - float(candidate.center[2])) > 1e-9:
        return action_center
    floor_top = float(container["thickness"]) + float(
        container.get("buffer", 0.0)
    )
    if abs(float(candidate.minimum[2]) - floor_top) <= _production.CONTACT_TOLERANCE:
        action_center[2] += 0.020
    return action_center


def _release_candidate(candidate, container):
    return AABB(
...
def transport_samples(candidate, container, *args, **kwargs):
    return _production_transport_samples(
        _release_candidate(candidate, container), container, *args, **kwargs
    )


def transport_sweeps(candidate, container, *args, **kwargs):
    return _production_transport_sweeps(
        _release_candidate(candidate, container), container, *args, **kwargs
    )
