...
### 解消（2026-08-06、利用者の承認を得て実施）

`experiment/anchor-recall-oracle` を作業ブランチの内容まで進めた。**ただし
fast-forward ではなくマージになった。**

承認を取る前の確認では「厳密な祖先＝FF可能」と出ていたが、それは**古い
リモート追跡 ref を見ていた**。実行直前に `git fetch` したところ trunk 側が
3コミット進んでおり、そこに**公式回答**が入っていた
(`transport-planes-are-unconditional-official`: 搬送参照面は `require_shelf`
に関わらず常に適用される、という SIGNATE 裁定と、それを固定する
`tests/test_transport_plane_contract.py` 122行)。FF していたら消えていた。

- **教訓はそのまま §A の再演である。** 「祖先だから安全」も、測った時点が
  古ければ嘘になる。ref の鮮度は主張の一部である。
- 台帳の衝突は**台帳自身の契約どおり**に解いた —「マージは id による和集合。
  順序に意味は無いが CLI の表示順なので追記する」。1件追記して 161 件。
  `contract` ブロックは長い方（作業ブランチ側）を残した。trunk 側はまだ
  1文しか無い旧版で、`replicates` も加法マージ規則も持っていない。
- 入ってきた `test_transport_plane_contract.py` 7件は、この枝の agent に対して
  そのまま green。全体 573 tests OK。

**残る判断:** `main` は凍結のまま触っていない（bootstrap 2コミットを持つため
FF にならず、「main は凍結」という現行方針の変更になる）。
...
1. §G の第4点（`submission22`, 17.581）を較正に足す。成分間の取引を
   決着させる唯一の道で、`attr-guard` はそこで止まっている。
2. §I の `rollout` / `physics` は、問いを登録するかノブを畳むか。放置は
   「仮説なき計装」のまま。
3. §A の残り: `main` を凍結のままにするか追従させるかを決める。急がない。
