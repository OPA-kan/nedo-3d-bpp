...
**F1. 飽和チェック（最優先・追加の物理runなしで実行できる）**

既存の33 replay snapshot 上で class別 \(\hat R_c\) を計算する。\(A(s)\) と同様に
\(\hat R_c\) もほぼ全snapshotで同値なら、理論は課題Cにおいて空振りである。
安く死ぬ経路をここに置く。期待する識別は、終端直前snapshotで \(\hat R_c\) と
\(H(s)\) が下がることである。

**F2. ラベルの張り替え**

placed-to-go ではなく、
(i) その状態以降に不可逆失敗まで何step残ったか、
(ii) 次に到着した荷物が安全に置けたか、
を予測対象にする。課題Cは到着順が固定なので直接測れる。
