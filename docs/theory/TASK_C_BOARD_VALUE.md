...
## 0.1 実装簡約の階層（2026-08-02、測定により改訂）

本書は当初 Hall 型競合を実装対象として書いた。測定の結果、それは**棄却では
なく降格**である。正確には次の3段の簡約になる。

**一般形.** 将来荷物型と利用可能資源の対応は class–resource 競合として記述され、
Hall 型量が状態の脆弱性を表しうる。これは一般論として正しい。

**この問題で観測された構造.** 公開7型の近傍集合は**強い入れ子構造**を持つ。
大きい型が入る場所には小さい型も入るため、絶滅順序はほぼサイズ順になった
（c001-k1 step 18/19 の実測、evidence `task-c-class-extinction-is-nested`）。
入れ子な系では Hall 欠損は最小近傍だけで決まるので、**一般 Hall 量は action 間で
定数**になる。実際そうなった。ただし cardboard（接地小・高さ0.40 m）が高さ軸で、
backpack_large と duffel が寸法軸で順序を崩すので、全順序ではなく
**低次元の半順序**である。

**実装対象.** 状態を候補点ではなく、幾何から構成した**到達可能ポケット**の
Pareto 前線で表す。各ポケット \(z\) は \(q(z)=(w_z,l_z,h_z,r_z,\sigma_z)\) を持ち、
型 \(c\) に対する同時余裕

\[
\mu_c(z)=\min\Bigl\{\tfrac{w_z-w_c}{w_c},\ \tfrac{l_z-l_c}{l_c},
\ \tfrac{h_z-h_c}{h_c},\ m_r(z,c),\ m_\sigma(z,c)\Bigr\}
\]

を独立ポケット間で降順に取って \(\mu_{c,1}\ge\mu_{c,2}\ge\cdots\) とする。
\(\mu_{c,1}\le0\) が絶滅、\(\mu_{c,1}>0\wedge\mu_{c,2}\le0\) が唯一の逃げ道、
\(\mu_{c,2}>0\) が代替経路あり。**最初のテトリス論の「独立な代替」を、連続量として
回収する形である。**

### なぜ候補点を捨てるのか（2つの実測破綻）

1. **離散カウントは action を識別できない。** c001-k1 step 18 の物理的に安全な
   4候補は `extinct_classes` / `min_class_slots` / `concentration_max` が
   **全て同一値**だったが、horizon ラベルは分かれた（rank 0,1 → step 20、
   rank 2,3 → step 21）。相関が弱いのではなく、**分離しうる情報がゼロ**である。
2. **候補由来の slot 数は generator 密度に汚染される。** 同一盤面で
   daypack_small は候補2025/スロット15（135:1）、suitcase_medium は 44/2（22:1）。
   盤面の性質ではない。

したがってポケットは**候補位置からではなく幾何から**構成しなければならない。

### 段階分けと、現在の到達点

属性を一度に入れると失敗を帰属できないので段階を切る。

| 段階 | 属性 | 状態 |
|---|---|---|
| 1 | \((w_z,l_z,h_z)\)、セル単位プローブ | **不合格** |
| 2 | ＋アイテム単位の契約検証 | **不合格（別方向）** |
| 3 | ＋ \(r_z\)（corridor）, \(\sigma_z\)（支持品質） | 未着手 |

段階1は、oracle が両 generator の完全列挙で候補ゼロと判定した c001-k1 step 20 で
**5型を生存と誤判定**した。原因はセル単位の妥当性がアイテム単位に合成しないこと
で、特に corridor は荷物の接地寸法に依存するため 5 cm プローブでは代理にならない。

段階2でアイテム単位の契約検証（`Geometry.rejection_reason`）を入れると、到着物
suitcase_medium を正しく絶滅と判定した。しかし**陰性対照が落ちる**: step 19 の
到着物 suitcase_small を絶滅と判定したが、実際には物理的に安全な配置が42件ある。
**過大評価を過小評価に置き換えただけ**であり、この状態では段階1の「合格」も
情報を持たない（ほぼ常に絶滅と言う指標は、詰み状態と自明に一致する）。

診断済みの原因は2つ。ポケット矩形の**中心1点でしか検証していない**こと（矩形が
荷物より大きいとき、中心が悪くても別位置なら入る）。および
`SupportPlaneComponent.top` が成分内の**最大**高さなので、成分内の低い面に載る
実配置を取り逃がすこと。

**現時点でポケット margin は忠実な予測子ではない。** 陰性対照を通過するまで、
ranking はもちろん shadow 計測にも載せない。
...
### 13.1 致命snapshot oracle の結果（2026-08-02）

`no_safe_action` を単一ラベルにすると「真に詰んだ」と「探索器が見つけられない」が
混ざるため、F2の前に致命stateを exhaustive oracle で分類した。結果は
`reports/task-c/fatal-oracle/summary.md`、evidence
`task-c-fatal-oracle-two-classes`。2つの致命snapshotは**1つずつ別クラス**だった。

| snapshot | クラス | 効くlever |
|---|---|---|
| c000-k1:21 | 真に詰み（両generator・無制限時間・物理判定で候補0） | 盤面価値のみ |
| c001-k1:18 | generator盲点（安全な候補60件を出荷generatorが構造的に見られない） | anchor被覆 |

したがって本理論の適用範囲が実測で確定した。**\(\Phi\) は c000-k1 型の死には
効き、c001-k1 型の死には原理的に効かない。**逆に anchor被覆の改善は
c001-k1 型にしか効かない。片方だけでは課題Cの死因は塞がらない。

この分割は §13 の主張（\(\Phi\) は選ぶときの質、fallbackは選べないとき）を、
さらに細かくしたものである。「選べないとき」の中に、
**本当に選べない**状態と**見えていないだけ**の状態がある。
