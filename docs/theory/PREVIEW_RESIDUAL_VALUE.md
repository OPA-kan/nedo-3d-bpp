...
### 9.8 段階A第2回結果(2026-07-31、較正済み測定器、陰性確定)

stride 16 × 3位相(較正§9.7で収束確認済み、accept cap非発火)での再実行:
baseline MAE 0.979 / R² 0.597 に対し descriptor追加で MAE 1.456 /
R² −0.241。残差Spearmanは全て |ρ|≤0.24。**V_cap(s) は placed-to-go を
baseline以上に説明しない。** 測定器は較正済みなので、これは測定欠陥では
なく仮説側の棄却である(n=24での過学習も含む)。

ただし機構的な副産物が重要:

1. **release受け入れ能力は最後まで枯渇しない。** 最大クラスの
   release cap_volume はほぼ全snapshot(終端直前を含む)で一定
   (0.113 / 0.080)、release anchor HT は数千のまま。エピソードは
   「置き場所が尽きて」終わるのではない。
2. **settled枯渇は終了を意味しない。** settled系列が0になった状態
   (b000-k20 step10以降)からも5配置続いた。
3. したがってこのデータでは **エピソード終了は容量制約ではなく
   物理失敗(release転倒)で決まる**。placed-to-go は容量ではなく
   「危険なreleaseがいつ転ぶか」の生存時間を測っている。

含意: 状態レベルの \(V_{\mathrm{cap}}(s)\) を placed-to-go で検証する
枠組み自体がこの環境の終了機構と噛み合っていない。将来価値の効き所は
行動別の「この配置が以後の受け入れ構造をどれだけ壊すか」であり、
それは転倒リスク(\(R_{\mathrm{phys}}\))と因果的に結合している
(転倒した箱が支持面を壊す)。段階B(同一snapshot内の候補間差)を
